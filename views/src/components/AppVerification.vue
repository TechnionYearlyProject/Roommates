      ...
      <v-flex>
        <v-card class="pb-3">
          <v-card-title primary-title>
            <div>
              <h3 class="headline mb-4 secondary--text"> You need to verify your account to start using all the fun features!</h3>
              <div class="mb-4">
              ...
              <v-text-field label="Password" prepend-icon="lock" v-model="payload.password" :append-icon="showPassword ? 'visibility' : 'visibility_off'" :append-icon-cb="() => (showPassword = !showPassword)" :type="showPassword ? 'text' :'password'" required></v-text-field>
            </div>
          </v-card-title>
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="secondary" @click="sendVerificationMail" dark>{{ resendButtonText }}
